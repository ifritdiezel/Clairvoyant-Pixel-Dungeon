...
public abstract class Wand extends Item {
	...
	private static float TIME_TO_ZAP	= 1f;
	...
	protected void wandUsed() {
		...
		Invisibility.dispel();
		updateQuickslot();
		if ( curUser.buff(Motivation.class) != null) TIME_TO_ZAP = 1f - curUser.pointsInTalent(Talent.PHILOSOPHILE)*0.2f;
					...
					if (curWand.cursed){
						...
						curWand.fx(shot, () -> {
							...
							if (curUser.subClass==HeroSubClass.CHANNELLER && Random.Int(5)==0 && curWand.curCharges > 4){
									...
									new EmoIcon.Alert(curUser.sprite);
							}
