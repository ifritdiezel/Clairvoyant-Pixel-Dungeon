...
public class Heap implements Bundlable {
	...
	public void burn() {
		...
		for (Item item : items.toArray( new Item[0] )) {
			if (item instanceof Scroll && !item.unique) {
				...
				items.remove( item );
				evaporated = true;
			} else if (item instanceof MysteryMeat || item instanceof FrozenCarpaccio || item instanceof StewedMeat) {
		...
		if (type != Type.HEAP) {
			...
			for (Item item : items.toArray( new Item[0] )) {
				...
				//upgraded items can endure the blast
					...
					Talent.onItemDestroyed( Dungeon.hero, pos);
					items.remove( item );
				}
	...
	public void freeze() {
		...
		for (Item item : items.toArray( new Item[0] )) {
			if (item instanceof MysteryMeat || item instanceof ChargrilledMeat || item instanceof StewedMeat) {
				replace( item, FrozenCarpaccio.cook( item.quantity ) );
				frozen = true;
