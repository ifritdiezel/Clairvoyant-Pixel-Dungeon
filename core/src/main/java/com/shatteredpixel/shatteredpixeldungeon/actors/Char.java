...
public abstract class Char extends Actor {
	...
	public boolean attack( Char enemy, float dmgMulti, float dmgBonus, float accMulti ) {
		...
		if (enemy.isInvulnerable(getClass())) {
			...
			if (this instanceof Hero){
				...
				if (h.belongings.weapon!=null && h.subClass == HeroSubClass.CHANNELLER) {
					channellerbonus = damageRoll()/(5-h.pointsInTalent(Talent.FORCE_CONVERSION));
					dmg4chan-=channellerbonus;
				}
			}
			...
			effectiveDamage = Math.max( effectiveDamage - dr, 0 )+ channellerbonus;
			...
			if (visibleFight) {
				if (effectiveDamage > 0 && effectiveDamage <100 || !enemy.blockSound(Random.Float(0.96f, 1.05f))) {
	...
	public static boolean hit( Char attacker, Char defender, float accMulti ) {
		...
		if (attacker.buff(  Motivation.class) != null) acuRoll *= 1.2f;
