...
import com.shatteredpixel.shatteredpixeldungeon.actors.buffs.Motivation;
...
public enum Talent {
	...
	//Apprentice T1
	SNACK_SAVING(0), APPRENTICES_INTUITION(1), PHILOSOPHILE(2), MAGIC_SHIELD(3),
	//Apprentice T2
	UNLEASHING_MEAL(4), MATTER_DISSIPATION(5), OPTIMIZED_TRANSMUTATION(6), ENERGY_REDIRECT(7, 0), ARCANE_STABILIZATION(8),
	//Apprentice T3
	RUNE_IMBUEMENT (9, 3), FORCE_CONVERSION(10, 3),
	//Berserker T3
	ENDLESS_RAGE(11, 3), BERSERKING_STAMINA(12, 3), ENRAGED_CATALYST(13, 3),
	//Channeller T3
	EXTERNAL_CATALYST(14, 3), LETHAL_DEFENSE(15, 3), ENHANCED_COMBO(16, 3),
	...
	public static void onTalentUpgraded( Hero hero, Talent talent){
		...
		if (talent == APPRENTICES_INTUITION && hero.pointsInTalent(APPRENTICES_INTUITION) == 2){
	...
	public static void onFoodEaten( Hero hero, float foodVal, Item foodSource ){
		if (hero.hasTalent(SNACK_SAVING) && (foodVal >= 300 ))  {
			switch (Random.Int(4+hero.pointsInTalent(SNACK_SAVING))) {
				case 3:
					new ChargrilledMeat().quantity(1).collect();
					break;
				case 4:
					new StewedMeat().quantity(1).collect();
					break;
				case 5:
					new SmallRation().quantity(1).collect();
					break;
				}
		}
		if (hero.hasTalent(EMPOWERING_MEAL)){
			//2/3 bonus wand damage for next 3 zaps
			Buff.affect( hero, WandEmpower.class);
		...
		if (hero.hasTalent(UNLEASHING_MEAL)){
			if (Random.Int(2) == 0) {
				Buff.affect(hero, FireImbue.class).set(3 * (hero.pointsInTalent(UNLEASHING_MEAL)));
			} else {
				FlavourBuff.prolong( hero, FrostImbue.class, ( 4*(hero.pointsInTalent(UNLEASHING_MEAL))));
			}
	...
	public static float itemIDSpeedFactor( Hero hero, Item item ){
		...
		// 2x/instant for Apprentice (see onItemEquipped)
		if (item instanceof Ring || item instanceof Wand){
			factor *= 1f + hero.pointsInTalent(APPRENTICES_INTUITION)*0.5f;
		}
	...
	public static void onItemDestroyed(Hero hero, int pos) {
		if (hero.hasTalent(MATTER_DISSIPATION)) {
			GameScene.add(Blob.seed(pos, 200*hero.pointsInTalent(MATTER_DISSIPATION), SmokeScreen.class));
		}
	}
	...
	public static void onItemEquipped( Hero hero, Item item ){
		if (hero.pointsInTalent(APPRENTICES_INTUITION) == 2 && (item instanceof Weapon || item instanceof Armor)){
			item.identify();
		}
	...
	public static void onItemCollected( Hero hero, Item item ){
		...
		if (hero.pointsInTalent(APPRENTICES_INTUITION) == 2){
	...
	public static void onItemIdentified(Hero hero){
		if (hero.hasTalent(PHILOSOPHILE)){
			...
			if (e != null) e.burst(Speck.factory(Speck.STAR), hero.pointsInTalent(PHILOSOPHILE));
		}
	...
	public static void initClassTalents( HeroClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents ){
		...
		switch (cls){
			case APPRENTICE: default:
				Collections.addAll(tierTalents, SNACK_SAVING, APPRENTICES_INTUITION, PHILOSOPHILE, MAGIC_SHIELD);
				break;
			case MAGE:
		...
		switch (cls){
			case APPRENTICE: default:
				Collections.addAll(tierTalents, UNLEASHING_MEAL, MATTER_DISSIPATION, OPTIMIZED_TRANSMUTATION, ENERGY_REDIRECT, ARCANE_STABILIZATION);
				break;
			case MAGE:
		...
		switch (cls){
			case APPRENTICE: default:
				Collections.addAll(tierTalents, RUNE_IMBUEMENT , FORCE_CONVERSION);
				break;
			case MAGE:
	...
	public static void initSubclassTalents( HeroSubClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents ){
		...
		switch (cls){
			case CHANNELLER:
				Collections.addAll(tierTalents, EXTERNAL_CATALYST);
				break;
			case BATTLEMAGE:
