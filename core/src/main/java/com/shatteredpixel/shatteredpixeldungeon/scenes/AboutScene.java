...
public class AboutScene extends PixelScene {
	...
	public void create() {
		...
		CreditsBlock cvpd = new CreditsBlock(true, Window.WHITE,
				"Clairvoyant Pixel Dungeon",
				Icons.IFRIT.get(),
				"Developed by: _ifritdiezel_\nBased on Shattered Pixel Dungeon's open source",
				"github.com/ifritdiezel",
				"https://github.com/ifritdiezel");
		cvpd.setRect((w - fullWidth)/2f, 6, 120, 0);
		content.add(cvpd);

		CreditsBlock boby = new CreditsBlock(false, Window.BOBY,
				"Coding help:",
				Icons.TRASHBOX_BOBYLEV.get(),
				"Trashbox Bobylev",
				"/u/TrashboxBobylev",
				"https://reddit.com/u/TrashboxBobylev");
		boby.setSize(colWidth/2f, 0);
		if (landscape()){
			boby.setPos(cvpd.right(), cvpd.top() + (cvpd.height() - boby.height())/2f);
		} else {
			boby.setPos(w/2f - colWidth/2f, cvpd.bottom()+5);
		}
		content.add(boby);
		...
		CreditsBlock shpx = new CreditsBlock(true, Window.SHPX_COLOR,
				...
				shpxLink);
		if (landscape()){
			shpx.setRect(cvpd.left(), cvpd.bottom() + 8, colWidth, 0);
		} else {
			shpx.setRect(shpx.left(), boby.bottom() + 8, colWidth, 0);
		}
		content.add(shpx);
		...
		CreditsBlock freesound = new CreditsBlock(true,
				...
				"_You probably misread_ the _Line above_\n" +
